...
import { BulkUploadService, SystemAndPart } from '../../../../swagger';
...
@Component({
  selector: 'app-system-part',
  templateUrl: './system-part.component.html',
  styleUrl: './system-part.component.scss',
})
export class SystemPartComponent {
  showLoader: boolean = false;
  systemandparts: SystemAndPart[] = [];
  showUploadDialog: boolean = false;
  uploadedFiles: File[] = [];

  expectedColumnsVendorMaster: string[] = ['System_Name', 'Part_Name'];
  ...
  ngOnInit(): void {
    this.loadSystemAndParts();
  }

  async loadSystemAndParts() {
    this.showLoader = true;
    try {
      const res = await firstValueFrom(
        this.bulkUploadService.bulkUploadGetSystemAndPartMasterGet()
      );
      if (res && res.length) {
        this.systemandparts = res;
      }
    } catch (err: any) {
      console.error(err);
    }
  ...
  async onUpload(event: any) {
    ...
    formData.append('fileType', 'SystemPart');
  ...
  downloadSampleFile() {
    let filePath = 'assets/sample/systempart_master.csv';
        ...
        this.papa.parse(fileContent, {
          ...
          complete: (result) => {
            ...
            if (
              ...
              console.log('CSV is valid for System And Part Master');
